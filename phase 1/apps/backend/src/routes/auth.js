...
const authRoutes = async (server) => {
    server.post('/signup', async (request, reply) => {
        try {
            const data = auth_1.SignupSchema.parse(request.body);
    ...
    server.post('/login', {
        ...
        try {
            const data = auth_1.LoginSchema.parse(request.body);
            ...
            const isValid = await argon2_1.default.verify(user.passwordHash, data.password);
            if (!isValid) {
                await new Promise(r => setTimeout(r, 1000));
                return reply.status(401).send({ error: 'Invalid credentials' });
            }
    ...
    server.post('/refresh', async (request, reply) => {
        try {
            const data = auth_1.RefreshSchema.parse(request.body);
    ...
    server.post('/logout', { preValidation: [authenticate] }, async (request, reply) => {
        try {
            const data = auth_1.LogoutSchema.parse(request.body);
    ...
    server.post('/logout-all', { preValidation: [authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.sub;
